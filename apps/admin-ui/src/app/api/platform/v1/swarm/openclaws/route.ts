...
import { listPlatformSwarmMembers, readPlatformClawSwarmStatus } from '@/lib/server/platform-clawswarm';
...
export async function GET(request: NextRequest) {
  ...
  const [service, members] = await Promise.all([
    ...
    listPlatformSwarmMembers(session.user.id),
  ]);

  return NextResponse.json({ ok: true, data: { service, members } });
}
